...
export const APP_KNOWLEDGE = [
  ...
  "Ask itself. Ask — the Claudia button, bottom right of every screen — answers from the tools behind the screens the person holds, never from anything else, and writes nothing on its own: a draft opens the app's own form, a send, a schedule, a chase or a reminder waits for the card's confirm. A reminder is a notification at a time on the person's own devices, set through Ask. It remembers the thread only until sign-out. The microphone dictates through the browser and stores nothing."
].join("\n");
