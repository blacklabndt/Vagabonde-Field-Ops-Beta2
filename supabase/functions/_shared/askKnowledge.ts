...
export const APP_KNOWLEDGE = [
  ...
  "Ask itself. Ask — the Claudia button, bottom right of every screen — answers questions about records from the tools behind the screens the person holds, never from anything else, explains how the app works, and helps with general tasks outside the app's records (wording a message, laying out a spreadsheet, planning a day) from general knowledge; it cannot browse or reach any other app. It writes nothing on its own: a draft opens the app's own form, a send, a schedule, a chase or a reminder waits for the card's confirm. A reminder is a notification at a time on the person's own devices, set through Ask. It remembers the thread only until sign-out. What it learns — facts about the app and task methods the crew teaches it — is one memory the whole crew shares; each answer shows what was kept, with a × to forget it, and list_learned shows it all. A note is one short sentence; a password, key, phone number or email address typed into the chat is not kept in one. The microphone dictates through the browser and stores nothing."
].join("\n");
