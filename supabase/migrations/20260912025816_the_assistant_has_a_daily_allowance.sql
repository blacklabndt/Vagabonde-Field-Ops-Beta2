...
-- ── the ceiling ────────────────────────────────────────────────────────────
-- Null is no ceiling, which is today's behaviour, so applying this migration
-- on its own changes nothing. The default is deliberately generous: this is a
-- stop on a runaway, not a budget the crew must work inside.
alter table public.app_settings
  add column if not exists ask_daily_token_cap bigint;
...
update public.app_settings set ask_daily_token_cap = 10000000
  where id and ask_daily_token_cap is null;
