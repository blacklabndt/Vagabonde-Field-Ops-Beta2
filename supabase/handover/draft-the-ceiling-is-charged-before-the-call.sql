-- DRAFT — NOT APPLIED, NOT HISTORY.
--
-- This file waits under supabase/handover/ because the session that wrote it
-- had no way to reach the database: no psql, no Supabase CLI login, no MCP
-- tool. Migrations are applied LIVE FIRST — the timestamp comes from the
-- applier — and only then filed under supabase/migrations/ with that version,
-- so that the repo and the applied history reconcile 1:1. Filing it under
-- migrations unapplied would break that reconciliation silently, which is
-- worse than waiting.
--
-- HOW TO SHIP IT:
--   1. Apply this file live.
--   2. Run probes-draft-the-ceiling-is-charged-before-the-call.sql beside it,
--      INCLUDING the two-session procedure at its foot — the whole ceiling
--      rests on two concurrent reservations not both passing, and a single
--      connection cannot ask that question.
--   3. Move this file to supabase/migrations/<applied version>_the_ceiling_
--      is_charged_before_the_call.sql.
--   4. ONLY THEN wire the function to it. ask/index.ts still calls
--      ask_record_spend and ask_allowance's two-column shape; this migration
--      drops the first and changes the second, so deploying the function
--      before the migration, or this migration before the function, breaks
--      Ask either way. The function change is NOT in the repo yet, on
--      purpose: a ceiling with a PGRST202 fallback is a ceiling anybody can
--      switch off by breaking a function name.
--
